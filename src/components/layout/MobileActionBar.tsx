...
export function MobileActionBar() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--border)] bg-[var(--surface)] px-3 py-2 pb-[calc(.5rem+env(safe-area-inset-bottom))] shadow-2xl transition-colors lg:hidden">
      <div className="mx-auto grid max-w-sm grid-cols-2 gap-2">
        {actions.map(({ label, href, icon: Icon }) => (
          <a key={label} href={href} className={label === "Pricing" ? "brand-gradient flex min-h-12 flex-col items-center justify-center rounded-xl text-xs font-extrabold text-white shadow-md" : "flex min-h-12 flex-col items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--brand-soft)] text-xs font-bold text-[var(--brand-deep)] transition hover:bg-[var(--brand-light)]"}>
